 ...
 * Plugin URI:  http://izaacj.dynu.com/
 * Description: Adds some options for the Twenty Seventeen theme, and even some more to our own childtheme of Twenty Seventeen.
 * Version:     1.1.0
 * Author:      Izaac Johansson
 * Author URI:  http://izaacj.dynu.com/
...
class izz0_advanced_2017
{
    ...
    public static function register($wp_customize)
    {
        ...
        if( izz0_advanced_2017::is_izz0ware_theme() ) {
        ...
        if( izz0_advanced_2017::is_izz0ware_theme() ) {
            $wp_customize->add_control(new WP_Customize_Control(
                ...
                array(
                    'type' => 'checkbox',
                    'label' => __('Show full height header on blog page.', 'izz0adv2017'),
                    'section' => 'izz0adv2017_options',
                )
            ));
        }
    ...
    public static function add_scripts() {
        if( get_theme_mod( 'full_blog_header' ) ) {
            wp_dequeue_script('twentyseventeen-global');
            wp_enqueue_script('twentyseventeen-izz0-global', plugin_dir_url(__FILE__) . 'assets/js/global.js', array( 'jquery' ), '1.0', true);
            wp_enqueue_style('twentyseventeen-izz0-blog-header', plugin_dir_url(__FILE__) . 'assets/css/styles.css');
        }
    }

    public static function body_classes($classes) {
        if( is_home() && get_theme_mod( 'full_blog_header' ) ) {
            $classes[] = 'home';
        }
...
if ('twentyseventeen' == $theme) {
    ...
    if( izz0_advanced_2017::is_izz0ware_theme() ) {
        // Actions only taken with our modified version of Twentyseventeen
        add_action( 'wp_enqueue_scripts', array( 'izz0_advanced_2017', 'add_scripts' ), 99999 );
        add_filter( 'body_class', array( 'izz0_advanced_2017', 'body_classes' ) );
    }
} else {
